...
public sealed class TfIdfVectorizer
{
    ...
    /// <summary>
    /// Gets the vocabulary mapping token string to token id.
    /// </summary>
    public IReadOnlyDictionary<string, int> Vocabulary => _tokenToId;
    /// <summary>
    /// Gets the inverse document frequency per token id.
    /// </summary>
    public IReadOnlyDictionary<int, double> Idf => _idf;

    /// <summary>
    /// Fits the vectorizer on items and returns normalized TF-IDF vectors.
    /// </summary>
    /// <param name="items">Sequence of items with token streams.</param>
    /// <returns>List of item vectors aligned with the learned vocabulary.</returns>
